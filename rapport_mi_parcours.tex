...
\section{Le point sur nos résultats}
    ...
    Pour assurer l'efficacité de l'attaque précédente, il faut faire en sorte qu'il y ait des requêtes DNS. Pour cela, il faut interrompre les connexions TCP existantes. Nous avons écrit un programme, toujours avec \verb!scapy!, qui envoie un paquet demandant la fin de la communication au demandeur.

    Cependant, ce programme est trop lent pour réellement interrompre la communication~: la vrai réponse arrive avant la nôtre, ce qui la rend obsolète. Nous avons deux hypothèses de travail~:
    \begin{itemize}[label=\color{bleu303}\textbullet{}]
